...
void teste_derivada (double (*f) (double), double (*fl) (double), double x)
{
  for (int i=1; i < 13; ++i) {
    double h = pow(10,-i);
    double d = -1; // COMPLETAR: calcular derivada
    double a = fl(x);
    printf("%e  %.16g  %.16g\n",h,d,fabs(d-a));
  }
}
...
{
  double s16 = -1; // COMPLETAR: chamar simpson com 16 passos
  double s32 = -1; // COMPLETAR: chamar simpson com 32 passos
  printf("%.16g  %.16g\n",s16,s32);
}
...
void teste_adaptativo (double (*f) (double), double a, double b, double d)
{
  for (int i = 1; i < 13; ++i) {
    double tol = pow(10,-i);
    double s = -1; // COMPLETAR: chamar simpson adaptativo em [a,b]
    printf("%e %.16g %e\n", tol, s, fabs(d-s));
  }  
}
